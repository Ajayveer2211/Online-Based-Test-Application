...
const API_URL = 'http://localhost:5000/exams';
...
const examService = { 
  ...
  saveExamResult: async (resultData) => {
    ...
    const response = await axios.post('http://localhost:5000/results', resultData, config);
    return response.data;
  },
  ...
  getUserResults: async (userId) => {
    ...
    const response = await axios.get(`http://localhost:5000/results/${userId}`, config);
    return response.data;
  },  
}
