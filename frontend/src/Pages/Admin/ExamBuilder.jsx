...
export default function ExamBuilder({ onSave, onCancel, initialExam }) {
  // Exam ke naam, time, aur questions ke liye state
  ...
  // Agar edit mode me aaya toh initial data set karo
  useEffect(() => {
    if (initialExam) {
      setExamName(initialExam.name);
      setTotalTime(initialExam.totalTime || 0);
      setQuestions(
        initialExam.questions || [{ question: '', choices: ['', ''], correct: '' }]
      );
    }
  }, [initialExam]);
  ...
  // Choice delete karna (minimum 2 choices required)
  const deleteChoice = (qIndex, cIndex) => {
    ...
    if (updated[qIndex].choices.length > 2) {
      ...
      toast.warning('Har question me kam se kam 2 choices honi chahiye.');
    }
  };
  ...
  const validateExam = () => {
    ...
    for (let q of questions) {
      if (!q.question.trim() || q.choices.length < 2 || !q.correct.trim()) {
        return false;
      }
  ...
  const handleSave = () => {
    if (!validateExam()) {
      toast.warning(
        'Saare fields fill karo aur total time > 0 hona chahiye.'
      );
      return;
    }
    onSave({
        ...
        <div className="card-body">
          ...
          <div className="mb-3">
            ...
            <input
              ...
              value={totalTime}
              onChange={(e) =>
                setTotalTime(Math.max(0, parseInt(e.target.value) || 0))
              }
              min="1"
            />
          </div>
            ...
            <div key={qIndex} className="mb-4 border p-3 rounded">
              ...
              <div>
                ...
                <input
                  ...
                  value={q.correct}
                  onChange={(e) =>
                    handleQuestionChange(qIndex, 'correct', e.target.value)
                  }
                />
              </div>
            </div>
